<div class='p-5'>
    <div class='mb-5 w-max rounded-lg p-3 mx-auto flex justify-center bg-white items-center  shadow-md'>
        <h1 class='w-max bg-white text-lg font-bold text-blue-900'><i class="fa-solid fa-bag-shopping"></i> QUẢN LÝ QUẢN TRỊ VIÊN</h1>
    </div>
        ...
        <table class="w-full mx-auto text-sm text-left rtl:text-right dark:text-gray-400">
            <thead class="text-xs text-gray-800 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr class=" bg-gradient-to-br from-slate-800 to-blue-950 text-white">
                    ...
                    <th scope="col" class="px-6 py-3 text-center"> <a href="?role=admin&controller=customer&action=create&check=staff" id="add" class="px-2 py-2 bg-green-500 text-white rounded-md block hover:bg-green-700"><i class="fa-solid fa-plus"></i> Thêm quản trị viên</a>
                    </th>
                </tr>
            </thead>
            <?php
    ...
    <?php
                foreach ($value as $a => $b) {
                    if ($a == "customer_pass") continue;
                    if ($a == "customer_role") {
                        echo $b == 1 ? "<td class='px-6 py-4'>Nhân viên</td>" : "<td class='px-6 py-4'>Quản lý</td>";
                    } else if ($a == "customer_img") echo "<td class='block w-20 h-20 overflow-hidden'><img class='w-full p-1' src='uploads/img/$b'></td>";
                    else
    ...
    <td class='px-6 py-4'>
        <div class='flex items-center justify-center gap-10'>
            <a class="edit text-white px-3 py-1 bg-blue-700 rounded-md" href="?role=admin&controller=customer&action=edit&id=<?= $value['customer_id'] ?>&check=staff"><i class="fa-solid fa-pen-to-square"></i></a>
            <a onclick="return confirm('Các sản phẩm nằm trong danh mục này đều sẽ bị xoá. Bạn có chắc chắc muốn xoá không?')" class="text-red-600 px-3 py-1 bg-red-50 border border-red-600 rounded-md" href="?role=admin&controller=customer&action=delete&id=<?= $value['customer_id'] ?>&check=staff"><i class="fa-solid fa-trash-can"></i></a>
        </div>
    </td>
<?php
                echo "</tr>";
